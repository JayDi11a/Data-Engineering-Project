...
def create_initial_dataframe(spark_session):
    ...
    try:
        ...
        df = (
            ...
            .option("subscribe", "current_headlines")
...
def start_streaming(df_parsed, spark):
    """
    Starts the streaming to table spark_streaming.current_headline in postgres
    """
    # Read existing data from PostgreSQL
    existing_data_df = spark.read.jdbc(
        POSTGRES_URL, "current_headlines_table", properties=POSTGRES_PROPERTIES
    )

    unique_column = "publishedat"
    ...
    query = df_parsed.writeStream.foreachBatch(
        lambda batch_df, _: (
            ...
            .write.jdbc(
                POSTGRES_URL, "current_headlines_table", "append", properties=POSTGRES_PROPERTIES
            )
        )
    ).trigger(once=True) \
        .start()

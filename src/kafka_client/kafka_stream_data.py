...
def update_last_processed_file(data: List[dict]):
    ...
    publication_dates_as_timestamps = [
        ...
        #datetime.fromisoformat(row["publishedAt"][:-1]).astimezone(timezone.utc).strftime('%Y-%m-%d')
        for row in data
    ]
    last_processed = max(publication_dates_as_timestamps) - datetime.timedelta(days=1)
    last_processed_as_string = last_processed.strftime("%Y-%m-%dT%H:%M:%SZ")
...
def get_all_data(last_processed_timestamp: datetime.datetime) -> List[dict]:
    ...
    while True:
        ...
        if n_results + MAX_LIMIT >= MAX_OFFSET:
            ...
            last_timestamp = current_results[-1]["publishedAt"]
            #timestamp_as_date = timestamp_as_date.strftime('%Y-%m-%d')
            timestamp_as_date = datetime.datetime.strptime(last_timestamp, "%Y-%m-%dT%H:%M:%SZ")
            timestamp_as_date = timestamp_as_date - datetime.timedelta(days=1)
            last_processed_timestamp = timestamp_as_date.strftime("%Y-%m-%dT%H:%M:%SZ")
            n_results = 0
...
def deduplicate_data(data: List[dict]) -> List[dict]:
     return list({v["publishedAt"]: v for v in data}.values())
...
def stream():
    ...
    for kafka_data in results:
        producer.send("current_headlines", json.dumps(kafka_data).encode("utf-8"))
